Вот версия в формате **Markdown** для вставки в **Obsidian**:
...
## 🎯 Пример: 312. Burst Balloons

- Состояния: отрезки `left`, `right` → `O(n²)`
- Варианты внутри: все `k` между `left` и `right` → `O(n)`
- Общая сложность: `O(n³)`, память `O(n²)`

---

> 💡 Храни это как файл `algo-complexity-cheatsheet.md` в своей Obsidian-заметке.
```

Хочешь, чтобы я добавил также **английскую версию**, или сделал **таблицу в виде PNG/обложки** для распечатки/вставки?
